...
bool Classic::DumpCardMemory(char s8_KeyType, const byte* u8_Keys, bool b_ShowAccessBits)
{
    ...
    assertDo (u8_UidLen == 0, LL_ERROR, LM_RFIDCLASSIC, "No card present", return false;);
    ...
    for (int B=0; B<64; B++)
    {
        ...
        {
            log(LL_DEBUG, LM_RFIDCLASSIC, "Sector:", (uint32_t) B/4);
        ...
        log(LL_DEBUG, LM_RFIDCLASSIC, "Block:", (uint32_t) B);
        
        if (ReadDataBlock(B, u8_Data))
        {
            log_hexdump(LL_DEBUG, LM_RFIDCLASSIC, "Data:", 16, u8_Data);
            ...
            if (GetValue(u8_Data, &u32_Value, &u8_Address))
            {
                ...
                log(LL_DEBUG, LM_RFIDCLASSIC, s8_Buf);
            }
...
void Classic::ShowAccessBits(byte u8_Block, byte u8_Byte7, byte u8_Byte8)
{
    ...
    for (int B=0; B<3; B++)
    {
        switch (u8_Access[B])
        {
            case 0: sprintf(s8_Buf, "Access block %02d: read data: key A|B, write data: key A|B", u8_Block); break;
            case 1: 
            case 2: sprintf(s8_Buf, "Access block %02d: read data: key A|B, write data: never",   u8_Block); break;
            case 3: sprintf(s8_Buf, "Access block %02d: read data: key B,   write data: key B",   u8_Block); break;
            case 4: 
            case 6: sprintf(s8_Buf, "Access block %02d: read data: key A|B, write data: key B",   u8_Block); break;
            case 5: sprintf(s8_Buf, "Access block %02d: read data: key B,   write data: never",   u8_Block); break;
            case 7: sprintf(s8_Buf, "Access block %02d: read data: never,   write data: never",   u8_Block); break;
        }
        log(LL_DEBUG, LM_RFIDCLASSIC, s8_Buf);
        u8_Block ++;
    }
    ...
    {
        case 0: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: key A\r\nAccess BITS: read: key A,   write: never\r\nAccess KEYB: read: key A,   write: key A"); break;
        case 1: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: key A\r\nAccess BITS: read: key A,   write: key A\r\nAccess KEYB: read: key A,   write: key A"); break;
        case 2: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: never\r\nAccess BITS: read: key A,   write: never\r\nAccess KEYB: read: key A,   write: never"); break;
        case 3: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: key B\r\nAccess BITS: read: key A|B, write: key B\r\nAccess KEYB: read: never,   write: key B"); break;
        case 4: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: key B\r\nAccess BITS: read: key A|B, write: never\r\nAccess KEYB: read: never,   write: key B"); break;
        case 5: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: never\r\nAccess BITS: read: key A|B, write: key B\r\nAccess KEYB: read: never,   write: never"); break;
        case 6: 
        case 7: log(LL_DEBUG, LM_RFIDCLASSIC, "Access KEYA: read: never,   write: never\r\nAccess BITS: read: key A|B, write: never\r\nAccess KEYB: read: never,   write: never"); break;
    }
}
...
{
    log(LL_DEBUG, LM_RFIDCLASSIC, "AuthenticateDataBlock()");
...
{
    log(LL_DEBUG, LM_RFIDCLASSIC, "ReadDataBlock()");
...
{
    log(LL_DEBUG, LM_RFIDCLASSIC, "WriteDataBlock()");
...
bool Classic::DataExchange(byte u8_Command, byte u8_Block, byte* u8_Data, byte u8_DataLen)
{
    ...
    assertDo (len < 3 || mu8_PacketBuffer[1] != PN532_COMMAND_INDATAEXCHANGE + 1, LL_ERROR, LM_RFIDCLASSIC, "DataExchange failed", return false;);
    ...
    {
        assertDo (len < 19, LL_ERROR, LM_RFIDCLASSIC, "DataExchange returned invalid data", return false;);
