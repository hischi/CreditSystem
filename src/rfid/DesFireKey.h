...
public:
    ...
    {
        assertDo(s32_ByteCount < ms32_BlockSize || s32_ByteCount % ms32_BlockSize, LL_ERROR, LM_DESKEY, "Invalid CBC block size", return false;);
    ...
    {
        log_hexdump(LL_DEBUG, LM_DESKEY, "PrintIV:", ms32_BlockSize, mu8_IV);
    }

    static bool CheckValid(DESFireKey* pi_Key)
    {
        assertDo(pi_Key == NULL || pi_Key->GetKeyType() == DF_KEY_INVALID, LL_WARNING, LM_DESKEY, "Invalid key", return false;);
    ...
    void PrintKey(const char* s8_LF=NULL)
    {
        ...
        log_hexdump(LL_DEBUG, LM_DESKEY, "Key:", GetKeySize(16), mu8_Key);
        log(LL_DEBUG, LM_DESKEY, "KeyAsString:", GetKeyTypeAsString(me_KeyType, ms32_KeySize));
    }
