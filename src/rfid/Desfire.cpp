...
{
    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        log(LL_DEBUG, LM_DESFIRE, "Authenticate(..) KeyNo:", (uint32_t) u8_KeyNo);
        pi_Key->PrintKey();
    }
    ...
    { 
        ...
        default:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Invalid key");
            return false;
    }
    ...
    assertDo (e_Status != ST_MoreFrames || (s32_Read != 8 && s32_Read != 16), LL_ERROR, LM_DESFIRE, "Authentication failed (1)", return false;);
    ...
    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndB_enc:", s32_RandomSize, u8_RndB_enc);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndB:", s32_RandomSize, u8_RndB);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndB_rot:", s32_RandomSize, u8_RndB_rot);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndA:", s32_RandomSize, u8_RndA);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndAB:", 2*s32_RandomSize, i_RndAB);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndAB_enc:", 2*s32_RandomSize, i_RndAB_enc);
    }
    ...
    assertDo (e_Status != ST_Success || s32_Read != s32_RandomSize, LL_ERROR, LM_DESFIRE, "Authentication failed (2)", return false;);
    ...
    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndA_enc:", s32_RandomSize, u8_RndA_enc);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndA_dec:", s32_RandomSize, u8_RndA_dec);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "RndA_rot:", s32_RandomSize, u8_RndA_rot);
    }

    // Last step: Check if the received random A is equal to the sent random A.
    assertDo (memcmp(u8_RndA_dec, u8_RndA_rot, s32_RandomSize) != 0, LL_ERROR, LM_DESFIRE, "Authentication failed (3)", return false;);
    ...
    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        log(LL_DEBUG, LM_DESFIRE, "SessKey: see below");
...
{
    log(LL_DEBUG, LM_DESFIRE, "ChangeKey(..) KeyNo:", (uint32_t) u8_KeyNo);

    assertDo (mu8_LastAuthKeyNo == NOT_AUTHENTICATED, LL_WARNING, LM_DESFIRE, "Not authenticated", return false;);

    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        log(LL_DEBUG, LM_DESFIRE, "SessKey IV: see below");
        mpi_SessionKey->PrintIV(LF);
        log(LL_DEBUG, LM_DESFIRE, "New Key: see below");
    ...
    if (!b_SameKey) 
    {
        ...
        if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
        {
            log(LL_DEBUG, LM_DESFIRE, "Cur Key: see below");
    ...
    log_hexdump(LL_DEBUG, LM_DESFIRE, "CRC Crypto (HEX):", 4, (uint8_t*) &u32_Crc);
    ...
    if (!b_SameKey)
    {
        ...
        log_hexdump(LL_DEBUG, LM_DESFIRE, "CRC New Key (HEX):", 4, (uint8_t*) &u32_CrcNew);
    }
    ...
    log_hexdump(LL_DEBUG, LM_DESFIRE, "Cryptogram:", s32_CryptoLen, i_Cryptogram);
    log_hexdump(LL_DEBUG, LM_DESFIRE, "Cryptog_enc:", s32_CryptoLen, u8_Cryptogram_enc);
...
{
    char s8_Buf[80];
    log(LL_DEBUG, LM_DESFIRE, "GetKeyVersion(..) KeyNo:", (uint32_t) u8_KeyNo);
    ...
    log_hexdump(LL_DEBUG, LM_DESFIRE, "Version (HEX):", 1, pu8_Version);
    return true;
}
...
{
    log(LL_DEBUG, LM_DESFIRE, "GetCardVersion()");
    ...
    if (checkLogLevel(LM_DESFIRE, LL_INFO))
    {
        char s8_Buf[80];
        log(LL_INFO, LM_DESFIRE, "--- Desfire Card Details ---");
        sprintf(s8_Buf, "Hardware Version: %d.%d", pk_Version->hardwareMajVersion, pk_Version->hardwareMinVersion);
        log(LL_INFO, LM_DESFIRE, s8_Buf);
        sprintf(s8_Buf, "Software Version: %d.%d", pk_Version->softwareMajVersion, pk_Version->softwareMinVersion);
        log(LL_INFO, LM_DESFIRE, s8_Buf);
        sprintf(s8_Buf, "EEPROM size:      %d byte", 1 << (pk_Version->hardwareStorageSize / 2));
        log(LL_INFO, LM_DESFIRE, s8_Buf);
        sprintf(s8_Buf, "Production:       week %X, year 20%02X", pk_Version->cwProd, pk_Version->yearProd);
        log(LL_INFO, LM_DESFIRE, s8_Buf);
        log_hexdump(LL_INFO, LM_DESFIRE, "UID no:          ", 7, pk_Version->uid);  
        log_hexdump(LL_INFO, LM_DESFIRE, "Batch no:        ", 5, pk_Version->batchNo);         
    }
    return true;
}
...
{
    log(LL_DEBUG, LM_DESFIRE, "FormatCard()");
...
{
    log(LL_DEBUG, LM_DESFIRE, "GetKeySettings(..)");
    ...
    *pe_KeyType   = (DESFireKeyType)(u8_RetData[1] & 0xF0);
    ...
    {
         char s8_Buf[80];
         sprintf(s8_Buf, "Settings: 0x%02X, KeyCount: %d, KeyType: %s", *pe_Settg, *pu8_KeyCount, DESFireKey::GetKeyTypeAsString(*pe_KeyType));
         log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
    return true;
}
...
{
    log_hexdump(LL_DEBUG, LM_DESFIRE, "ChangeKeySettings(..) New Setting:", 1, (uint8_t*) &e_NewSettg);
...
{
    log(LL_DEBUG, LM_DESFIRE, "EnableRandomIDForever()");
...
{
    log(LL_DEBUG, LM_DESFIRE, "GetRealCardID()");
    ...
    log_hexdump(LL_DEBUG, LM_DESFIRE, "CRC (HEX):", 4, (uint8_t*) &u32_Crc2);

    assertDo (u32_Crc1 != u32_Crc2, LL_WARNING, LM_DESFIRE, "Invalid CRC", return false;);

    log_hexdump(LL_DEBUG, LM_DESFIRE, "Read UID:", 7, u8_UID);
...
{
    log(LL_DEBUG, LM_DESFIRE, "GetFreeMemory(..)");
    ...
    *pu32_Memory = i_Data.ReadUint24();
    ...
    log(LL_DEBUG, LM_DESFIRE, "Free memory (n bytes):", *pu32_Memory);
    return true;
}
...
{
    log(LL_DEBUG, LM_DESFIRE, "GetApplicationIDs(..)");
    ...
    *pu8_AppCount = (s32_Read1 + s32_Read2) / 3;
    ...
    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        if (*pu8_AppCount == 0)
        {
            log(LL_DEBUG, LM_DESFIRE, "No Application ID's");
        }
        else for (byte i=0; i<*pu8_AppCount; i++)
        {
            char s8_Buf[80];
            sprintf(s8_Buf, "Application %2d: 0x%06X", i, (unsigned int)u32_IDlist[i]);
            log(LL_DEBUG, LM_DESFIRE, s8_Buf);
        }
    }
    return true;
...
bool Desfire::CreateApplication(uint32_t u32_AppID, DESFireKeySettings e_Settg, byte u8_KeyCount, DESFireKeyType e_KeyType)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "CreateApplication(App= 0x%06X, KeyCount= %d, Type= %s)", (unsigned int)u32_AppID, u8_KeyCount, DESFireKey::GetKeyTypeAsString(e_KeyType));
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }

    assertDo (e_KeyType == DF_KEY_INVALID, LL_ERROR, LM_DESFIRE, "Invalid key type", return false;);
...
bool Desfire::DeleteApplication(uint32_t u32_AppID)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "DeleteApplication(0x%06X", (unsigned int)u32_AppID);
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
...
bool Desfire::SelectApplication(uint32_t u32_AppID)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "SelectApplication(0x%06X)", (unsigned int)u32_AppID);
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
...
{
    log(LL_DEBUG, LM_DESFIRE, "GetFileIDs(..)");
    ...
    *pu8_FileCount = s32_Read;
    ...
    if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
    {
        if (*pu8_FileCount == 0)
        {
            log(LL_DEBUG, LM_DESFIRE, "No files.");
        }
        else 
        {
            log_hexdump(LL_DEBUG, LM_DESFIRE, "File ID's:", s32_Read, u8_FileIDs);
        }
    }
    return true;
}
...
bool Desfire::GetFileSettings(byte u8_FileID, DESFireFileSettings* pk_Settings)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "GetFileSettings(ID= %d)", u8_FileID);
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
    ...
    switch (pk_Settings->e_FileType)
    {
        ...
        case MDFT_BACKUP_DATA_FILE:
            ...
            log(LL_DEBUG, LM_DESFIRE, "FileSize:", pk_Settings->u32_FileSize);
            return true;
...
bool Desfire::CreateStdDataFile(byte u8_FileID, DESFireFilePermissions* pk_Permis, int s32_FileSize)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "CreateStdDataFile(ID= %d, Size= %d)", u8_FileID, s32_FileSize);
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
...
{
    log(LL_DEBUG, LM_DESFIRE, "DeleteFile(..) ID:", (uint32_t) u8_FileID);
...
bool Desfire::ReadFileData(byte u8_FileID, int s32_Offset, int s32_Length, byte* u8_DataBuffer)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "ReadFileData(ID= %d, Offset= %d, Length= %d)", u8_FileID, s32_Offset, s32_Length);
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
...
bool Desfire::WriteFileData(byte u8_FileID, int s32_Offset, int s32_Length, const byte* u8_DataBuffer)
{
    ...
    {
        char s8_Buf[80];
        sprintf(s8_Buf, "WriteFileData(ID= %d, Offset= %d, Length= %d)", u8_FileID, s32_Offset, s32_Length);
        log(LL_DEBUG, LM_DESFIRE, s8_Buf);
    }
                          ...
                          DESFireCmac    e_Mac)               // in
{
    ...
    assertDo (2 + pi_Command->GetCount() + pi_Params->GetCount() > PN532_PACKBUFFSIZE || s32_Overhead + s32_RecvSize > PN532_PACKBUFFSIZE, LL_ERROR, LM_DESFIRE, "DataExchange(): Invalid parameters", return -1;);

    if (e_Mac & (MAC_Tcrypt | MAC_Rcrypt))
    {
        assertDo (mu8_LastAuthKeyNo == NOT_AUTHENTICATED, LL_ERROR, LM_DESFIRE, "Not authenticated", return -1;);
    }
    ...
    if (e_Mac & MAC_Tcrypt) // CRC and encrypt pi_Params
    {
        if (checkLogLevel(LM_DESFIRE, LL_DEBUG))
        {
            log(LL_DEBUG, LM_DESFIRE, "Sess Key IV: see below");
            mpi_SessionKey->PrintIV(LF);
        }    
        ...
        log_hexdump(LL_DEBUG, LM_DESFIRE, "CRC Params (HEX):", 4, (uint8_t*) &u32_Crc);
        log_hexdump(LL_DEBUG, LM_DESFIRE, "Params (HEX):", s32_CryptCount, pi_Params->GetData());
    
        if (!mpi_SessionKey->CryptDataCBC(CBC_SEND, KEY_ENCIPHER, pi_Params->GetData(), pi_Params->GetData(), s32_CryptCount))
            return -1;
    
        log_hexdump(LL_DEBUG, LM_DESFIRE, "Params_enc (HEX):", s32_CryptCount, pi_Params->GetData()); 
    }
    ...
    { 
        ...
        log_hexdump(LL_DEBUG, LM_DESFIRE, "TX CMAC (HEX):", mpi_SessionKey->GetBlockSize(), u8_CalcMac); 
    }
    ...
    assertDo (s32_Len < 3 || mu8_PacketBuffer[1] != PN532_COMMAND_INDATAEXCHANGE + 1, LL_ERROR, LM_DESFIRE, "DataExchange() failed", return -1;);
        ...
        (mu8_LastAuthKeyNo != NOT_AUTHENTICATED))                          // No session key -> no CMAC calculation possible
    {
           ...
           (u8_CardStatus == ST_Success)) // Response contains CMAC only in case of success
        {
            ...
            log_hexdump(LL_DEBUG, LM_DESFIRE, "RX CMAC (HEX):", mpi_SessionKey->GetBlockSize(), u8_CalcMac); 
      
            // For AES the CMAC is 16 byte, but only 8 are transmitted
            assertDo (memcmp(u8_RxMac, u8_CalcMac, 8) != 0, LL_ERROR, LM_DESFIRE, "CMAC Mismatch", return -1;);
        }
    }

    assertDo (s32_Len > s32_RecvSize, LL_ERROR, LM_DESFIRE, "DataExchange() Buffer overflow", return -1;)
    ...
    if (u8_RecvBuf && s32_Len)
    {
        ...
        if (e_Mac & MAC_Rcrypt) // decrypt received data with session key
        {
            ...
            log_hexdump(LL_DEBUG, LM_DESFIRE, "Decrypt:", s32_Len, u8_RecvBuf);
        }    
    }
    return s32_Len;
}
...
bool Desfire::CheckCardStatus(DESFireStatus e_Status)
{
    ...
    {
        case ST_OutOfMemory:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Not enough EEPROM memory.");
            return false;
        case ST_IllegalCommand:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Illegal command.");
            return false;
        case ST_IntegrityError:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Integrity error.");
            return false;
        case ST_KeyDoesNotExist:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Key does not exist.");
            return false;
        case ST_WrongCommandLen:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Wrong command length.");
            return false;
        case ST_PermissionDenied:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Permission denied.");
            return false;
        case ST_IncorrectParam:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Incorrect parameter.");
            return false;
        case ST_AppNotFound:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Application not found.");
            return false;
        case ST_AppIntegrityError:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Application integrity error.");
            return false;
        case ST_AuthentError:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Authentication error.");
            return false;
        case ST_LimitExceeded:
           assertCnt(true, LL_ERROR, LM_DESFIRE, "Limit exceeded.");
            return false;
        case ST_CardIntegrityError:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Card integrity error.");
            return false;
        case ST_CommandAborted:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Command aborted.");
            return false;
        case ST_CardDisabled:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Card disabled.");
            return false;
        case ST_InvalidApp:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Invalid application.");
            return false;
        case ST_DuplicateAidFiles:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "Duplicate AIDs or files.");
            return false;
        case ST_EepromError:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "EEPROM error.");
            return false;
        case ST_FileNotFound:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "File not found.");
            return false;
        case ST_FileIntegrityError:
            assertCnt(true, LL_ERROR, LM_DESFIRE, "File integrity error.");
            return false;
        default:
            log(LL_ERROR, LM_DESFIRE, "Unknown error: ", (uint32_t) e_Status);
            assertInc();
            return false;
    }
}
...
{
    log(LL_INFO, LM_DESFIRE, "Selftest success");
    ...
    assertDo ((e_CardType & CARD_Desfire) == 0, LL_ERROR, LM_DESFIRE, "The selftest requires a Desfire card.", return false;);
    ...
    assertDo (u8_Version != 0, LL_ERROR, LM_DESFIRE, "The selftest requires an empty Desfire card (factory default DES key)", return false;);
    ...
    assertDo (u8_AppCount != 4 || u32_IDlist[0] != u32_App2KDES || u32_IDlist[1] != u32_App3KDES || u32_IDlist[2] != u32_AppAES || u32_IDlist[3] != u32_AppDel,LL_ERROR,LM_DESFIRE, "GetApplicationIDs() failed", return false;);
    ...
    assertDo (u8_AppCount != 3,LL_ERROR,LM_DESFIRE, "DeleteApplication() failed", return false;);
    ...
    assertDo (e_Settg != KS_FACTORY_DEFAULT || u8_KeyCount != 2 || e_KeyType != DF_KEY_2K3DES, LL_ERROR,LM_DESFIRE, "GetKeySettings() failed", return false;);
    ...
    assertDo (u8_FileCount != 1 || u8_FileIDs[0] != 5, LL_ERROR,LM_DESFIRE, "GetFileIDs() failed", return false;);
    ...
    assertDo (k_Settings.e_FileType      != MDFT_STANDARD_DATA_FILE ||
        k_Settings.e_Encrypt       != CM_PLAIN ||
        k_Settings.k_Permis.Pack() != k_Permis.Pack() ||
        k_Settings.u32_FileSize    != FILE_LENGTH, LL_ERROR,LM_DESFIRE, "GetFileSettings() failed", return false;);
    ...
    assertDo (memcmp(u8_TxData, u8_RxData, FILE_LENGTH) != 0, LL_ERROR,LM_DESFIRE, "Read/Write file failed", return false;);
    ...
    assertDo (u8_FileCount != 0, LL_ERROR,LM_DESFIRE, "DeleteFile() failed", return false;);
...
{   
    log(LL_INFO, LM_DESFIRE, "SelftestKeyChange started");
    ...
    assertDo (e_Settg != 0x0D || u8_KeyCount != 2 || e_KeyType != pi_NewKeyA->GetKeyType(), LL_ERROR,LM_DESFIRE, "ChangeKeySettings() failed", return false;);             
    ...
    assertDo (u8_KeyVersion != CARD_KEY_VERSION, LL_ERROR,LM_DESFIRE, "GetKeyVersion() failed", return false;);    
    ...
    log(LL_INFO, LM_DESFIRE, "SelftestKeyChange success");
    return true;
}